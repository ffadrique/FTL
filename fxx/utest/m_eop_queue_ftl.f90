...
!---Public/Private declarations-------------------------------------------------
  ...
  public t_eop_queue_ftl, eop_queue_ftl
...
!   The element data instance
    class(t_eop),  pointer :: element => null()
...
! Linked queue container type
  type, extends(t_object) :: t_eop_queue_ftl
    private
...
! The input queue
  type(t_eop_queue_ftl), intent(in) :: this
...
! The input array
  class(t_eop), dimension(:), intent(in) :: a
...
! The queue
  type(t_eop_queue_ftl), intent(inout) :: this
...
! The queue
  class(t_eop_queue_ftl), intent(inout) :: this

! The element
  class(t_eop), intent(in) :: element
...
function queue_front( this ) result(res)

! The queue
  class(t_eop_queue_ftl), target, intent(in) :: this

! Pointer to the element in the first node in the queue
  class(t_eop), allocatable :: res
...
function queue_back( this ) result(res)
...
! The queue
  class(t_eop_queue_ftl), intent(in) :: this
...
! The output queue
  class(t_eop_queue_ftl), intent(out) :: this

! The input queue
  type(t_eop_queue_ftl), intent(in) :: other
...
! The input array
  class(t_eop), dimension(:), intent(in) :: array
...
! The returned array of elements (unallocated if memory failure)
  class(t_eop), allocatable, dimension(:) :: res
...
! Element to be allocated and assigned (pointer interface)
  class(t_eop), pointer, intent(inout) :: left

! Source element
  class(t_eop), intent(in) :: right
...
! Element to be allocated and assigned (allocatable interface)
  class(t_eop), allocatable, intent(inout) :: left
